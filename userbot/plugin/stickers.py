...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "BlackShadowBot"
...
async def _(event):
    ...
    #packname = f"BlackShadowBhai™keStickers"
    #packshortname = f"BlackShadowBot_{userid}_ns"  # format: Uni_Borg_userid
    if userid == 953414679:
        packname = f"BlackShadowBhaikeStickers"
        packshortname = "BlackShadowBhai_ke_locker_me"
    else:
        packname = f"{user.first_name}'s BlackShadowBot Vol.{pack}"
        packshortname = f"BlackShadowBot_{userid}_kang"
    await event.edit("`Look dat way,it's a gurl!\nMeanwhile, lemme kang this stcker over hehe ヽ༼ ಠ益ಠ ༽ﾉ`")

    is_a_s = is_it_animated_sticker(reply_message)
    file_ext_ns_ion = "@iaminnocentyaar.png"
    ...
    if is_a_s:
        ...
        if userid == 953414679:
            ...
            packshortname = "BlackShadowBhai_ke_Animated"
        else:
            packname = f"{user.first_name}'s BlackShadowBot Animated Vol.{pack}"
            packshortname = f"BlackShadowBot_{userid}" # format: Uni_Borg_userid
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                    ...
                    #if userid == 948408212:
                       # packname = f"{user.first_name}'s BlackShadowBot Vol.{pack}"
                       # packshortname = "Vol._{pack}_BlackShadowBhai_ke_locker_me"
                   # else:
                       # packname = f"Vol._{pack}_BlackShadowBot{userid}"
                        #packshortname = f"Vol._{pack}_BlackShadowBot_{userid}_ns"
                    if not await stickerset_exists(bot_conv, packshortname):
